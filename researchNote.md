...
차량의 좌표 변환 시스템을 설명하는 좋은 예로, 자율주행 차량에서 여러 센서 및 부품 간의 위치 및 방향 관계를 관리하는 ROS의 tf 시스템을 사용할 수 있습니다. 이 시스템은 각 부품이나 센서의 좌표계를 관리하며, 자율주행 차량이 자신이 처한 환경을 이해하고 제어하는 데 매우 중요한 역할을 합니다.
차량의 tf 시스템 예시:

자율주행 차량에는 여러 좌표계(Frame)가 존재합니다. 예를 들어:

    base_link: 차량의 본체 중심에 있는 좌표계입니다. 일반적으로 차량의 이동을 기준으로 하는 좌표계입니다.
    odom: 차량의 주행 경로를 추적하는 좌표계입니다. 차량이 이동함에 따라 축적되는 경로를 기준으로 좌표가 설정됩니다.
    map: 차량이 주행하는 전반적인 지도 좌표계입니다. GPS 등을 이용해 전역적인 위치를 추적합니다.
    camera_frame: 차량에 장착된 카메라의 좌표계입니다. 카메라의 센서에서 수집한 데이터를 tf 시스템을 통해 다른 좌표계로 변환합니다.
    lidar_frame: 라이다(LiDAR) 센서의 좌표계입니다. 라이다로 수집된 데이터를 base_link나 map 좌표계로 변환하여 주변 환경을 인식합니다.

차량에서 tf 토픽을 통한 변환 발행:

자율주행 차량에서 tf 시스템을 통해 다양한 센서 간의 좌표 변환을 관리하는 과정을 살펴보겠습니다.

    기본 좌표계 설정: 차량의 중심이 되는 좌표계는 **base_link**가 됩니다. 이 좌표계는 차량의 본체에서 기준으로 설정되며, 모든 다른 센서들이 base_link를 기준으로 상대적인 위치를 정의합니다.

    센서 좌표계 간 변환: 차량에 장착된 센서들, 예를 들어 카메라, 라이다, GPS 등은 각기 다른 좌표계를 가지고 있습니다. 이들 센서의 데이터를 차량의 중심 좌표계인 base_link로 변환하거나, 더 나아가 지도 좌표계(map)로 변환하는 과정이 필요합니다.
        예를 들어, 카메라로 수집한 이미지 데이터를 차량의 주행 경로에 맞춰 변환하려면 카메라의 좌표계(camera_frame)에서 base_link 또는 map 좌표계로 변환해야 합니다.

    동적인 좌표 변환 관리: 차량이 움직일 때마다 각 센서의 위치와 방향이 변하므로, 이러한 변화를 실시간으로 tf 시스템을 통해 브로드캐스트해야 합니다. 이를 위해 tf 토픽에 차량의 이동 정보가 주기적으로 퍼블리시됩니다. 차량이 앞으로 이동하거나 회전하면, 모든 센서들의 좌표계도 함께 변동되므로, 이러한 변환 정보를 지속적으로 갱신합니다.
...
class CarTransformPublisher : public rclcpp::Node {
public:
    CarTransformPublisher() : Node("car_transform_publisher") {
        // TransformBroadcaster 객체 생성
        tf_broadcaster_ = std::make_shared<tf2_ros::TransformBroadcaster>(this);
        
        // 주기적으로 변환을 발행하는 타이머 설정
        timer_ = this->create_wall_timer(
            std::chrono::milliseconds(100),  // 100ms 간격으로 발행
            std::bind(&CarTransformPublisher::publish_transform, this));
    }
...
private:
    void publish_transform() {
        ...
        transformStamped.header.frame_id = "base_link";  // 부모 프레임
        transformStamped.child_frame_id = "camera_frame";  // 자식 프레임

        // 변환 정보(Translation) 설정: 카메라가 차량에서 1미터 앞에 있는 경우
        transformStamped.transform.translation.x = 1.0;
        transformStamped.transform.translation.y = 0.0;
        transformStamped.transform.translation.z = 0.5;  // 카메라가 조금 위에 장착된 경우

        // 회전 정보(Rotation) 설정: 회전은 없다고 가정 (쿼터니언)
...
int main(int argc, char *argv[]) {
    ...
    auto node = std::make_shared<CarTransformPublisher>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
...
이 코드에서:
    ...
    base_link: 차량의 중심 좌표계입니다.
    camera_frame: 카메라가 설치된 위치를 나타내는 좌표계입니다. 카메라는 차량 앞쪽에 1미터 떨어진 곳에 설치되어 있습니다.
    이 프로그램은 차량이 이동할 때마다 카메라의 좌표계 정보를 /tf 토픽에 발행하여 다른 노드들이 카메라와 본체 간의 변환 정보를 활용할 수 있게 합니다.

이처럼 차량의 여러 센서들의 위치 관계를 tf 시스템을 통해 관리하고, /tf 토픽에 변환 정보를 발행함으로써 자율주행 차량의 다양한 기능을 지원할 수 있습니다.

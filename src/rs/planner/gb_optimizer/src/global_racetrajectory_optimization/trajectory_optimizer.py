...
def trajectory_optimizer(input_path: str,
    ...
    if not (curv_opt_type == 'mintime' and not mintime_opts["recalc_vel_profile_by_tph"]):
        file_paths["ggv_file"] = os.path.join(file_paths["inputs"], "veh_dyn_info", pars["ggv_file"].strip('"'))
        file_paths["ax_max_machines_file"] = os.path.join(file_paths["inputs"], "veh_dyn_info", pars["ax_max_machines_file"].strip('"'))

        # 디버깅용 출력 추가
        print("Constructed ggv_file path:", file_paths["ggv_file"])
        print("Constructed ax_max_machines_file path:", file_paths["ax_max_machines_file"])

...
MPPIControllerROS::MPPIControllerROS() : nh_(""), private_nh_("~"), tf_listener_(tf_buffer_) {
    ...
    sub_off_lim_ = nh_.subscribe("/off_lim", 1, &MPPIControllerROS::callback_off_lim, this);
...
void MPPIControllerROS::callback_off_lim(const std_msgs::Bool& msg) {
    if (msg.data) {
        off_lim_ = true;
    }
}
...
void MPPIControllerROS::timer_callback([[maybe_unused]] const ros::TimerEvent& te) {
    ...
    if (off_lim_ == false)  // off_lim flag가 true이면 속도 제한을 하지 않는다.
